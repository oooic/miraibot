...
def my_update():
    cmd = ["/usr/sge/bin/linux-x64/qstat",
           "-u", user, "|", "grep", "-v", "LowPri"]
    cmd = " ".join(cmd)
...
def memory_usage():
    ...
    df = pd.read_csv(StringIO(qhost), skiprows=3, sep="\s+", names=[
                     'node', 'os', 'cores', 'load', 'max_mem', 'used_mem', 'max_swap', 'used_swap'])

    for mem in 'max_mem', 'used_mem', 'max_swap', 'used_swap':
        df.loc[:, mem] = df[mem].str.replace(
            "M", "e3").str.replace("G", "e6").astype(float)
    ...
    df_qstat = pd.read_csv(StringIO(qstat), sep="\s+", names=['jobID',  'prior', 'name', 'user',  'state', 'date', 'time', 'queue', 'slots'])
